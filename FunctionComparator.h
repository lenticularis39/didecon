...
private:
  ...
  template<typename T> int LogReturnValueOptional(const char *Function,
        int Value, T *L, T *R) const {
    if (llvm::sys::fs::exists("/verbose-off"))
        return Value;
    if (llvm::sys::fs::exists("/verbose-bb"))
        return LogReturnValue(Function, Value, L, R);
    else
        return LogReturnValue(Function, Value);
  }

  template<typename T> int LogReturnRawValue(const char *Function, int Value,
		  T L, T R) const {
    if (llvm::sys::fs::exists("/verbose-off"))
        return Value;
    if (Value != 0 || llvm::sys::fs::exists("/verbose-dbg")) {
        ...
        if (L && R)
            errs() << "\tL: " << L << "\n\tR: " << R << "\n";
    } else if (llvm::sys::fs::exists("/verbose-dbg")) {
        errs() << "Function " << Function << " returned " << Value << "\n";
    }
    return Value;
  }

  int LogReturnValue(const char *Function, int Value) const {
    if (llvm::sys::fs::exists("/verbose-off"))
        return Value;
    if (Value != 0 || llvm::sys::fs::exists("/verbose-dbg"))
        errs() << "Function " << Function << " returned " << Value << "\n";
